...
class SiteController extends Controller
{
		...
		$model->status='0';
			if($model->save())
			{
				...
				//CVarDumper::dump($model->attributes,10,1);die;
					...
					{
					echo 'A Conformation mail has been sent to you. Check in your Inbox or Spam folder.';
					}
					else
					{
						echo ' Account Successfully Created!! Now you can logIn.';
					}
		...
		if(isset($_POST['ArtistTrack']))
			{
				...
				// CVarDumper::dump($_FILES,10,1);die;
				$userfile_extn = explode(".", strtolower($_FILES['ArtistTrack']['name']['song_url']));
				...
				//CVarDumper::dump($moveResult,10,1);die;				
	...
	public function actionUpdatePhoto($id)
	{
			...
			if(isset( $_FILES["Profile"]))
			{
			   ...
			   // CVarDumper::dump(CUploadedFile::getInstance($model,'image'),10,1);die;
		 ...
		 if(isset($_POST["Profile"]))
			{
				...
				if($model->save())
				  ...
				  //CVarDumper::dump(Yii::app()->user->role,10,1);die;
				  	if(Yii::app()->user->role=='1')
						$this->renderPartial('ajaxprofile',array('model'=>$model));
					else
						$this->renderPartial('ajaxlistener',array('model'=>$model));
	...
	public function actionSearch()
	{
		...
		if(isset($_POST['search']))
		{
			...
			'params' => array(':fuga' => '%'.$str.'%',':fug' => '%'.$str.'%'),));
			$result2= Cities::model()->findAll(array(
			'condition' => 'name LIKE :fuga OR name LIKE :fug',
			'params' => array(':fuga' => '%'.$_POST['search'].'%',':fug' => '%'.$str.'%'),));
	...
	public function actionSong($sid)
	{
		...
		$this->renderPartial('song',array('modelSong'=>$modelSong));
	}
	/*public function actionUpload()
{
	...
	$ext=substr($fname,strrpos($fname,'.')+1);
		...
		$newname=time().'.'.$ext;
		$path="songs/".$newname;
			if(move_uploaded_file(($_FILES["track"]["tmp_name"]) ,$path))
			{
			$model->users_id=$id;
			$model->song_name=$sname;
			$model->song_url=$newname;
			...
			{
			echo "Only 2MB File Allowed ! Thanks";
			}
		}
	}
...
*/		public function actionForgetpass()
		...
		public function actionPlaysongs()
			{
				...
				echo htmlspecialchars(trim('songs/',' ').$song->song_url);
			...
			public function actionAjaxlistener($id)
			{
				$model=Profile::model()->findByAttributes(array('users_id'=>$id));
			...
			{
				$id=$_POST['id'];
				$surl=htmlspecialchars(Yii::app()->createUrl("site/playsongs"));
				...
				echo "<tr><td width='100%' colspan='2' bgcolor='#333333'><h4 align='center' class='color-white'>".$songs[0]->usersPlaylist->playlist_name."</h4></td></tr>";
				echo "<tr bgcolor='#CCCCFF'><td colspan='2'>";
				echo '<ol class="list" style=" list-style-type: none;" >';
					foreach($songs as $al)
					{
						$slink=ArtistTrack::model()->findByPk($al->artist_track_id);
						echo "<li> <img src='".Yii::app()->theme->baseUrl."/img/play.png' class='small-icon' style='margin-top:-2px;' /> " ;
	...
	//View Lists 
	public function actionViewList($type,$id)
	{
		if($type=='follower')
		{
			$modelData=Follower::model()->findAllByAttributes(array('artists_profile_id'=>$id));
		}
		else if($type=='like')
		{
			$modelData=Follower::model()->findAllByAttributes(array('artists_profile_id'=>$id,'is_liked'=>1));
		}
		else if($type=='following')
		{
			$modelData=Follower::model()->findAllByAttributes(array('users_id'=>$id,'is_followed'=>1));
		
		}
		else if($type=='liked')
		{
			$modelData=Follower::model()->findAllByAttributes(array('users_id'=>$id,'is_liked'=>1));
		
		}
		$this->renderPartial('list',array('modelData'=>$modelData,'type'=>$type));
	}
}