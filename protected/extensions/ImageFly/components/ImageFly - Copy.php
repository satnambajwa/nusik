...
class ImageFly
{
	...
	public function getCustom($model, $attribute, $width = self::MEDIUM, $height = self::MEDIUM) {
		  $data=Albums::model()->findByAttributes(array('id'=>$model->albums_id));
		$model->data=$model->imagePathThumb.$data->name.'/';
       	//CVarDumper::dump($model->data,10,1);die;
	    $path = $this->getThumbNameCustom($model, $width, $height);
        ...
        if (!file_exists($this->pathRoot . $path . $c)) 
        {
            if(!is_dir($this->pathRoot . $model->imagePathThumb . $width . "_" . $height))
            {
                $listfolderName = explode("/", $model->data);
                $listfolderName[] = $width . "_" . $height;
                self::validCreateFoldersFiles($listfolderName, true, $this->file);
            }
            
            $image = $this->resizeAndSaveCustom($filePath.$imageName, $savePath, $c, $model, $width, $height);
        }
        else
        {
            if(!$this->validImage($filePath.$imageName, $c)){
                unlink($savePath.$c);
                $image = $this->resizeAndSaveCustom($filePath, $savePath, $c, $model, $width, $height);
            }else{
                $image = $this->webRoot . $path . $c;
            }
        }
	...
	//Customized for Albums
     private function resizeAndSaveCustom($imagePath, $savePath, $imageName, $model, $width, $height) {
        
        $canvas = canvas::Instance();
        $canvas->carrega($imagePath)
               ->redimensiona($width, $height, 'proporcional')
               ->grava($savePath.$imageName);
        CVarDumper::dump(($width, $height),10,1);die;
        return $this->webRoot.$this->getThumbNameCustom($model, $width, $height).$imageName;
    }
    /**
	...
	//Custmoized for Albums
	
	private function getThumbNameCustom($model, $width, $height) {
          $data=Albums::model()->findByAttributes(array('id'=>$model->albums_id));
//        CVarDumper::dump($model->imagePathThumb.$data->name.'/' . $width . '_' . $height . DIRECTORY_SEPARATOR,10,1);die;
		return $model->imagePathThumb.$data->name.'/' . $width . '_' . $height . DIRECTORY_SEPARATOR;
    }
    /**
    ...
    public function getPathCustome($model, $attribute, $location) {
        
        if ($model->$attribute instanceof CUploadedFile) {
            return "images".DIRECTORY_SEPARATOR.$model->imagePath.DIRECTORY_SEPARATOR.$location.DIRECTORY_SEPARATOR.time().".".$model->$attribute->getExtensionName();
        }
